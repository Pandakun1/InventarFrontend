...
import RetroDrawerDesign from './designs/RetroDrawerDesign.vue'
...
const currentComponent = computed(() => {
  switch (store.layoutKey) {
    case 'briefcase':
      return BriefcaseDesign
    case 'kidsShelf':
      return KidsShelfDesign
    case 'tacticalBackpack':
      return TacticalBackpackDesign
    case 'sciFiHud':
      return SciFiHudDesign
    case 'retroDrawer':
...
<template>
  <div
    :class="[
      'min-h-screen flex items-center justify-center p-8 app-root',
...
.ui-select-label {
  font-size: calc(var(--ui-scale, 1) * 10px);
...
.ui-select {
  ...
  padding: 4px 6px;
  border-radius: 6px;
  font-size: calc(var(--ui-scale, 1) * 11px);
  text-transform: uppercase;
